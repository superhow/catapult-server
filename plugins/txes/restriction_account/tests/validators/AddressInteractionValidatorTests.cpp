...
#define TEST_CLASS AddressInteractionValidatorTests
	...
	namespace {
		...
		Address ToAddress(const Key& publicKey) {
			return model::PublicKeyToAddress(publicKey, Default_Network);
		}
		...
		void PopulateCache(
			...
			for (const auto& pair : cacheContents) {
				auto sourceAddress = ToAddress(pair.first);
				...
				for (const auto& value : pair.second)
					TOperationTraits::Add(restriction, state::ToVector(ToAddress(value)));
			}
		...
		void AssertValidationResult(
			...
			auto notification = model::AddressInteractionNotification(ToAddress(source), entityType, participantsByAddress);
