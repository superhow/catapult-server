...
#define TEST_CLASS MultisigCosignatoriesObserverTests
	...
	namespace {
		...
		model::MultisigCosignatoriesNotification CreateNotification(
				const Address& multisig,
				const std::vector<Address>& addressAdditions,
				const std::vector<Address>& addressDeletions){
			return test::CreateMultisigCosignatoriesNotification(
					multisig,
					ToUnresolvedAddresses(addressAdditions),
					ToUnresolvedAddresses(addressDeletions));
		}
