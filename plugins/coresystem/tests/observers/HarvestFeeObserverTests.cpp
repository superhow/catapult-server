...
#define ACCOUNT_TYPE_TRAITS_BASED_TEST(TEST_NAME) \
	...
	namespace {
		model::BlockNotification CreateBlockNotification(const Key& harvester, const Key& beneficiary) {
			return test::CreateBlockNotification(
					model::PublicKeyToAddress(harvester, model::NetworkIdentifier::Zero),
					model::PublicKeyToAddress(beneficiary, model::NetworkIdentifier::Zero));
		}
	...
	ACCOUNT_TYPE_TRAITS_BASED_TEST(CommitCreditsHarvester) {
		...
		RunHarvestFeeObserverTest(NotifyMode::Commit, 0, [](auto& context, const auto& observer) {
			...
			auto notification = CreateBlockNotification(harvester, Key());
	...
	namespace {
		...
		void AssertHarvesterSharesFees(
			...
			RunHarvestFeeObserverTest(notifyMode, options, calculator, [&](auto& context, const auto& observer) {
				...
				auto notification = CreateBlockNotification(harvester, beneficiary);
